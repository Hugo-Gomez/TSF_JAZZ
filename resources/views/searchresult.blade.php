...
<ul class="searchresult">
	@if ($queryAgenda->isNotEmpty())
		@foreach ($queryAgenda as $querys)
			<li>• <a class="searchlink" href="{{ url('/jazzenda') }}">{{ $querys->title }}</a></li>
...
<ul class="searchresult">
	@if ($queryBlog->isNotEmpty())
		@foreach ($queryBlog as $querys)
			<li>• <a class="searchlink" href="{{ $querys->link }}" target="_blank">{{ $querys->author }}</a><span> {{ $querys->description }}</span></li>
		@endforeach
	@else
		<h1 style="text-align: center;">Pas de résultat dans cette catégorie</h1>
	@endif
...
<ul class="searchresult">
	@if ($queryNews->isNotEmpty())
		@foreach ($queryNews as $querys)
			<li>• <a class="searchlink" href="{{ url('/news'.$querys->id) }}">{{ $querys->title }}</a><span> ({{ date('d/m/Y', strtotime($querys->date)) }})</span></li>
		@endforeach
	@else
		<h1 style="text-align: center;">Pas de résultat dans cette catégorie</h1>
	@endif
...
<ul class="searchresult">
	@if ($queryPodcast_item->isNotEmpty())
		@foreach ($queryPodcast_item as $querys)
			<li >• <a class="searchlink" href="{{ url('/podcast'.$querys->id_podcast) }}">{{ $querys->title }}</a><span> <span class="red">par</span> {{ $querys->author }}</span> (Podcast du : {{ date('d/m/Y', strtotime($querys->date)) }})</li>
		@endforeach
	@else
		<h1 style="text-align: center;">Pas de résultat dans cette catégorie</h1>
	@endif
...
<ul class="searchresult">
	@if ($queryProgram_item->isNotEmpty())
		@foreach ($queryProgram_item as $querys)
			<li>• <a class="searchlink" href="{{ url('/program'.$querys->id_program) }}">{{ $querys->title }}</a><span> <span class="red">//</span> {{ $querys->subtitle }}</span></li>
		@endforeach
	@else
		<h1 style="text-align: center;">Pas de résultat dans cette catégorie</h1>
	@endif
